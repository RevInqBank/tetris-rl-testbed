...
**소스는 계속 ES module 이다. 번들과 `engine.js` 가 어긋나면 번들이 낡았다는 뜻이다.**
...
2. **afterstate 시뮬레이션.** `apply_placement(s, p)` 가 돌려주는 `next_state.current` 가
   ...
   **그래서 위 표의 "0개 미리보기"를 정보이론적 보장으로 읽으면 안 된다.**
   이 모드는 *"미래를 볼 수 있는가"* 를 제한하지 않는다.
   **"미래를 보려면 afterstate 시뮬레이터를 작성해야 하는가"** 를 제한할 뿐이다.
   그 정도가 실질적 장벽인지는 실험 설계자의 판단이며, **엔진은 그 이상을 보장하지 않는다.**
...
- **Dellacherie·CEM 은 next 를 안 보므로 익스트림에서도 성능이 변하지 않아야 한다.**
  변한다면 어딘가에서 next 를 몰래 쓰고 있다는 뜻이다. 이 모드의 부수적 검증 효과다.
