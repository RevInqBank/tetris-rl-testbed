...
> **알고리즘을 바꿔서가 아니라 문제를 다시 정의해서 성능이 올랐다.**
...
**마지막 줄이 이 표의 요점이다.**
문제 정의를 바꾸면 **전부** 효과가 있었고, **알고리즘을 더했을 때만 효과가 없었다.**
...
> **안 죽는 정책끼리는 "지운 줄 수"가 아무것도 구분하지 못한다.**
...
**즉 점수는 "얼마나 오래 버티나"가 아니라 "얼마나 잘 쌓나"를 묻는다.** 안 죽는 정책도 이 축에서는 갈린다.
...
**정책이 자주 터뜨리는 대신 자주 죽는 쪽을 골랐고**, 그 대가가 "팍 터뜨리기"다.
...
**④는 액터를 지웠다.** `π(a|s)` 대신 `a = argmax Q`. **파라미터가 0개인 액터**가 됐다.
...
> **CEM 이 이긴 것을 "진화전략이 강하다"로 읽으면 안 된다. "이 문제의 정책이 작다"는 뜻이다.**
...
> **이 넷은 §5 이전 설정, 즉 줄 수 목표로 학습한 정책이다.**
...
> AlphaZero 가 자기대국에 막대한 계산을 쓰는 이유가 여기 있다.
...
기존에도 우물 관련 특징이 하나 있었다. `cumulative_wells` 인데, **우물의 *비용*만 잰다.**
...
**"표현력이 병목이었다"는 유지된다. 다만 무엇의 병목이었는지를 갈라야 한다.**
...
**표현 수단이 생기자 정책이 실제로 우물 전략을 배웠다. 다만 오래 사는 것으로는 이어지지 않았다.**
...
> **첫 번째는 이상해 보였고**(세 전략이 같은 값) **두 번째는 그럴듯해 보였다.**
...
이 문서 서두의 논지와 뿌리가 같다. **문제 정의가 성능을 바꿨다**는 말은,
...
**사용자가 물었다.** 점수를 노리면 자주 죽고, 오래 살면 점수가 안 오른다.
**둘을 동시에 원하면 어떻게 되나.**
...
그쪽은 대신 점수를 버렸다. **여기서 고른 것은 "둘 다"가 아니라 둘의 균형이다.**
...
**정직하게 적는다. 이 절이 없으면 위 숫자가 실제보다 강하게 읽힌다.**
...
**그런데도 크레딧 할당에 대해서는 아무 말도 못 한다.** 두 문제에 순서가 있기 때문이다.
...
**이 개발기와 `REPORT.md` 는 같은 일을 다른 축으로 적었다.** 이 문서는 **막힌 지점**을 따라가고,
