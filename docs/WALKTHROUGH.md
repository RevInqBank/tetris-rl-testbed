...
**이 문서의 용도**: 이 프로젝트를 처음 보는 사람이 **이 문서 하나로 "무엇을 어떻게 만들었고 무엇을 알아냈는지"** 를 이해하게 하는 것이다.

강화학습을 공부해본 적은 있지만 이 프로젝트는 전혀 모르는 사람을 독자로 잡았다.
테트리스 규칙이나 REINFORCE·A2C·DQN 이 무엇인지는 설명하지 않는다.
대신 **이 프로젝트 안에서 생긴 결정과 용어는 전부 설명한다.**

**왜 하필 테트리스였나** — 만든 사람이 강화학습을 공부하면서 **"크리틱이 갱신식의 어디에 들어가는가"**
라는 축을 따라가고 있었다(REINFORCE → 기준선 → A2C → DQN → …). 그 축은 **알고리즘마다 다른 문제로
비교하면 아무 말도 못 한다.** 그래서 **하나의 문제 위에 그 알고리즘들을 나란히 세우는 실험대**가 필요했고,
테트리스가 그 자리에 놓였다. **게임을 만드는 것이 목적이 아니라, 게임이 실험대였다.**

| 다른 문서 | 무엇이 있나 |
|---|---|
| `docs/REPORT.md` | **결함 감사 기록** — 무엇이 틀렸고 어떻게 잡았는지. 열한 건 |
| `docs/TIMELINE.md` | 시각순 사건, 판정 이력, 폐기된 수치 |
| `PROJECT.md` | 팀이 합의한 설계 계약 |
...
| **ARENA** | **13개 전략이 동시에** 각자 판을 돌린다. 어느 정책이 어떻게 쌓는지 눈으로 비교한다 |
| **LEARN** | 각 전략이 "크리틱을 어디에 쓰는가"를 지도처럼 보여준다 |

```bash
bash tools/serve.sh        # 기본 포트 8080
# → http://localhost:8080/
```

**왜 13개인가.** 학습한 것 10종 + 학습 없는 대조군 2종(무작위, 사람이 손으로 정한 가중치) + 탐색 변형이다.
대조군이 있어야 "학습이 뭔가를 했는지"를 말할 수 있다.

**이 앱은 게임이 아니라 실험대다.** 사용자가 강화학습을 공부하면서
*"크리틱이 갱신식의 어디에 들어가는가"* 라는 축을 눈으로 보려고 만든 것이고, 패널 배치가 그 축을 그대로 따른다.
