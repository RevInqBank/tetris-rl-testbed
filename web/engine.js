...
import {
  W, BUFFER_ROWS, ROWS, FULL_ROW,
  PIECE_CELLS, UNIQUE_ROTS, SPAWN_X, SPAWN_Y,
  SCORE_TABLE, SOFT_DROP_POINTS_PER_CELL, HARD_DROP_POINTS_PER_CELL,
  GRAVITY_L1_L10, GRAVITY_TAIL, GRAVITY_MIN,
  LOCK_DELAY_MS, LOCK_RESET_LIMIT,
  B2B_LINES, B2B_MULT_NUM, B2B_MULT_DEN, COMBO_BONUS_PER_STEP,
  DIFFICULTY_NEXT_VISIBLE, DIFFICULTY_HOLD_ENABLED, DIFFICULTY_DEFAULT,
  DIFFICULTY_NAMES,
  LINES_PER_LEVEL,
  MIN_DX, MIN_DY, MAX_DY, X_RANGE, MAX_PIECE_VEXTENT, GUARD_ROWS,
  FNV_OFFSET_32, FNV_PRIME_32,
  XORSHIFT_FALLBACK_STATE, QUEUE_MIN, NEXT_VISIBLE,
  O as PIECE_O,
} from './tables.js';
...
import { KICKS, KICKS_NONE } from './tables.js';
export { KICKS, KICKS_NONE, KICKS_I, KICKS_JLSTZ } from './tables.js';

/** SRS kick candidates for a rotation, in test order. */
export function kickOffsets(piece, frm, to) {
...
export function framesPerCell(level) {
...
export const DEFAULT_RNG_STATE = XORSHIFT_FALLBACK_STATE;
...
export function stateFromObject(d) {
  const s = new State();
  s.rows = d.rows.slice();
  s.current = d.current;
  s.rot = d.rot;
 ...
 */
export function visibleNext(state) {
 ...
 */
export function newGame(seed = 1, difficulty = DIFFICULTY_DEFAULT) {
 ...
 */
export function fitsForParity(rows, piece, rot, x, y) {
 ...
 */
export function columnTops(rows) {
 ...
 */
export function scoreClearForParity(n, level, b2b, combo) {
  return scoreClear(n, level, b2b, combo);
}

export function applyPlacement(state, p) {
...
export function rotate(state, cw = true) {
  if (state.game_over || state.current === null) return false;
  const piece = state.current;
  const frm = state.rot;
 ...
 */
export function tickMs(state, dtMs) {
  if (state.game_over || state.current === null) return null;
...
export function lockDelayProgress(state) {
...
export function activeCells(state) {
...
export function ghostCells(state) {
  if (state.current === null) return [];
...
export function stateFromGame(game) {
  return game.state;
}
...
export function boardHash(rows) {
  let h = FNV_OFFSET >>> 0;
  for (let y = 0; y < ROWS; y++) {
    const r = rows[y];
...
export function renderAscii(state, includeBuffer = false) {
...
export const legal_placements = legalPlacements;
export const apply_placement = applyPlacement;
export const new_game = newGame;
export const board_hash = boardHash;
export const state_hash = stateHash;
/**
...
export const column_tops = columnTops;
export const frames_per_cell = framesPerCell;
export const soft_drop = softDrop;
export const hard_drop = hardDrop;
export const drop_distance = dropDistance;
export const ghost_y = ghostY;
export const kick_offsets = kickOffsets;
export const seed_state = seedState;
export const next_u32 = nextU32;
export const next_bag = nextBag;
export const from_dict = stateFromObject;
export const visible_next = visibleNext;
export const next_visible_count = nextVisibleCount;
export const hold_enabled = holdEnabled;
export const difficulty_name = difficultyName;
export const tick_ms = tickMs;
export const lock_delay_progress = lockDelayProgress;
export const placement_left_col = placementLeftCol;
export const placement_cells = placementCells;
export const gravity_interval_ms = gravityIntervalMs;
